...
TEST_CASE("erase left child") {
	SECTION("simple") {
		auto tree = tree::BinarySearchTree<int>({ 4, 2 });
	...
	SECTION("more compelx"){
		auto tree = tree::BinarySearchTree<int>({ 6, 4, 2, 1 });
		...
		tree = tree::BinarySearchTree<int>({ 8, -1, 2, 10, 4, 3, -3, 0 });
...
TEST_CASE("erase right child") {
	SECTION("simple") {
		auto tree = tree::BinarySearchTree<int>({7, 9});
		CHECK(tree.size() == 2);
		CHECK(tree.height() == 1);
	...
	SECTION("more complex") {
		auto tree = tree::BinarySearchTree<int>({8, 3, 1, -2, 0, 9, 12, 10 });
		/** tree looks like
		 *					  8
		 *				/			\
		 *			  3				  9
		...
		tree = tree::BinarySearchTree<int>({ 5, 8, 10});
		/** tree looks like
		 * .			5
		 *					\
		 *					8
...
TEST_CASE("erase case - both children") {
	...
	SECTION("simple, not root") {
		auto tree = tree::BinarySearchTree<int>({5, 3, 7, 2, 1, 6, 8});
		/** tree looks like
		 * .			5
		 *			/		\
		 *			3		7
	...
	SECTION("complex tree") {
		auto tree = tree::BinarySearchTree<int>({8, 2, 10, 9, 6, 4, -2, 12, 11, -4, -1, 1, 3, 7});
		CHECK(tree.size() == 14);
		CHECK(tree.height() == 4);
		/**	tree looks like 
		 * .							8
